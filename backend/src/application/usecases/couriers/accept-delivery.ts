...
import {
  CourierCapacityExceededError,
  CourierNotAvailableError,
  InvalidOrderStatusTransitionError,
  OrderNotFoundError,
} from "../../../domain/errors/domain-errors.js";
import { type CourierRepository, type OrderRepository } from "../../ports/repositories.js";

export type AcceptDeliveryDeps = Readonly<{
  couriers: CourierRepository;
  orders: OrderRepository;
}>;

export async function acceptDelivery(
  deps: AcceptDeliveryDeps,
  params: { readonly courierId: string; readonly orderId: string }
): Promise<
  ResultT<
    void,
    | CourierNotAvailableError
...
> {
  ...
  if (!courier || courier.status !== "AVAILABLE") return Result.err(new CourierNotAvailableError());
  ...
  if (courier.level === "STANDARD") {
    if (active.length >= 1) return Result.err(new CourierCapacityExceededError());
  } else {
    if (active.length >= 2) return Result.err(new CourierCapacityExceededError());
    if (active.length === 1) {
