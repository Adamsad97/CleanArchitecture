...
export type PostgresDb = {
  query<T extends QueryResultRow>(sql: string, params?: unknown[]): Promise<readonly T[]>;
  close(): Promise<void>;
};

export async function openPostgresDb(connectionString: string): Promise<PostgresDb> {
  const pool = new Pool({ connectionString });

  const postgresDatabase: PostgresDb = {
    async query<T extends QueryResultRow>(sql: string, params?: unknown[]): Promise<readonly T[]> {
...
async function initSchema(db: PostgresDb): Promise<void> {
  await db.query(`
...
async function ensureLegacyAccountsSchema(db: PostgresDb): Promise<void> {
...
async function ensureLegacyProfileTables(db: PostgresDb): Promise<void> {
  const accountRows = await db.query<{
    id: string;
    account_json: unknown;
  }>("SELECT id, account_json FROM accounts");

  for (const accountRow of accountRows) {
    const account = fromJson<{
      firstName?: unknown;
      lastName?: unknown;
      birthDate?: unknown;
      phone?: unknown;
...
async function ensureLegacyMenuItemsSchema(db: PostgresDb): Promise<void> {
  await db.query("ALTER TABLE menu_items ADD COLUMN IF NOT EXISTS image_url TEXT");
}

function getProfileTableName(role: unknown): "client_profiles" | "courier_profiles" | "restaurant_profiles" | null {
