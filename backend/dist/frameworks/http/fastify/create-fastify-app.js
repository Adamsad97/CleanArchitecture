...
export function createFastifyApp(deps) {
    ...
    app.post("/payments/card/visa/verify", async (req, reply) => {
        try {
            const body = z.object({ method: z.literal("CARD"), provider: z.literal("VISA"), cardNumber: z.string().regex(/^\d{16}$/), cardHolder: z.string().min(2), cardExpiry: z.string().regex(/^(0[1-9]|1[0-2])\/[0-9]{2}$/), cardCvc: z.string().regex(/^\d{3,4}$/) }).parse(req.body);
    ...
    app.post("/payments/card/mastercard/verify", async (req, reply) => {
        try {
            const body = z.object({ method: z.literal("CARD"), provider: z.literal("MASTERCARD"), cardNumber: z.string().regex(/^\d{16}$/), cardHolder: z.string().min(2), cardExpiry: z.string().regex(/^(0[1-9]|1[0-2])\/[0-9]{2}$/), cardCvc: z.string().regex(/^\d{3,4}$/) }).parse(req.body);
    ...
    app.post("/payments/paypal/verify", async (req, reply) => {
        try {
            const body = z.object({ method: z.literal("PAYPAL"), paypalEmail: z.string().email() }).parse(req.body);
    ...
    app.post("/payments/mobile-money/orange-money/verify", async (req, reply) => {
        try {
            const body = z.object({ method: z.literal("MOBILE_MONEY"), provider: z.literal("ORANGE_MONEY"), phoneNumber: z.string().min(8) }).parse(req.body);
    ...
    app.post("/payments/mobile-money/wave/verify", async (req, reply) => {
        try {
            const body = z.object({ method: z.literal("MOBILE_MONEY"), provider: z.literal("WAVE"), phoneNumber: z.string().min(8) }).parse(req.body);
    ...
    app.get("/orders/:id", async (req, reply) => {
        const params = z.object({ id: z.string().min(1) }).parse(req.params);
        const query = z.object({ clientId: z.string().min(1).optional() }).parse(req.query ?? {});
    ...
    app.get("/couriers/proposals", async () => {
        ...
        return {
            orders: orders.filter((order) => (order.fulfillmentType ?? DEFAULT_FULFILLMENT_TYPE) ===
