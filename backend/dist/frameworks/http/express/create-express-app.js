...
import { registerExpressPaymentRoutes } from "../payments/register-express-payment-routes.js";
const MAX_JSON_BODY_SIZE_MB = 10;
const JSON_BODY_SIZE_LIMIT = `${MAX_JSON_BODY_SIZE_MB}mb`;
...
export function createExpressApp(deps) {
    ...
    app.post("/auth/register", async (req, res) => {
        ...
        try {
            body = z
                ...
                if (payload.actorRole === "RESTAURANT")
                    return;
                if (!payload.birthDate || payload.birthDate.trim().length < 10) {
                    ctx.addIssue({
                        code: z.ZodIssueCode.custom,
                        path: ["birthDate"],
                        message: "birthDate is required for client and courier accounts",
                    });
                }
            })
                .parse(req.body);
        ...
        const result = await registerAccount({
            ...
            clientProfiles: deps.clientProfiles,
            courierProfiles: deps.courierProfiles,
            restaurantProfiles: deps.restaurantProfiles,
            ids: deps.ids,
            clock: deps.clock,
        }, {
            ...body,
    ...
    app.get("/accounts/:id/profile", async (req, res) => {
        const accountId = z.string().min(1).parse(req.params.id);
    ...
    registerExpressPaymentRoutes(app, deps);
    ...
    app.get("/orders/:id", async (req, res) => {
        ...
        if (!order) {
            return res.status(404).json({ status: 404, code: "ORDER_NOT_FOUND", message: "Commande introuvable" });
        }
        if (clientId && order.clientId !== clientId) {
            return res.status(403).json({ status: 403, code: "ORDER_FORBIDDEN", message: "Commande non accessible" });
        }
        res.json({ order });
    });
    app.get("/invoices/:id", async (req, res) => {
        ...
        if (!invoice)
            return res.status(404).json({ status: 404, code: "INVOICE_NOT_FOUND", message: "Facture introuvable" });
        res.json({ invoice });
    });
    // Restaurateur
